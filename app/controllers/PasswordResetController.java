...
public class PasswordResetController extends Controller {
    ...
    private static void tryAgainRequestReset(String email) {
        flash.put(Application.EMAIL, email);
        validation.keep();
        resetPassword();
    }
    ...
    public static void changePassword(String username, String uuid) {
        ...
        try {
            User user = UserService.fetchForPasswordReset(username, uuid);
            if (user == null) {
                showInvalidLinkFollowedPage();
            }

            renderArgs.put(Application.USERNAME, username);
            renderArgs.put(Application.UUID, uuid);
            render();
        } catch (Exception e) {
    ...
    private static void tryAgainChangePassword(String username, String uuid, String newPassword, String confirmPassword) {
        flash.put(Application.NEW_PASSWORD, newPassword);
        flash.put(Application.CONFIRM_PASSWORD, confirmPassword);
        validation.keep();
